...
function DiscardOverlapping(H_full_in, S_full_in, M_list_in; criterion="overlap", tol=0.01, kappa_max=1e10, verbose=false)
    ...
    for j=M:(-1):1
        ...
        for k=M_j:(-1):1
            ...
            if do_discard
                ...
                H_old = deepcopy(H_full)
                S_old = deepcopy(S_full)
                ...
                if verbose
                    ...
                    c[1:col-1] = C[1:col-1,1]
                    c[col+1:end] = C[col:end,1]
                    
                    println(transpose(c)*H_old*c)
                    println(transpose(c)*S_old*c)
        ...
        for s=1:2
            ...
            for i=1:M
                if i in twos
                    ...
                    push!(T1_oht, ITensor([1.0], inds(oht_list[i],tags="c")[1]))
                    push!(V_list, 1.0)
                end
            ...
            # Re-generate the new OHT states:
            T1_oht = [OHTCompIndexTensor(ITensor(ones(M1_list[i]), dag(inds(T1_oht[i])[1])) * T1_oht[i],discards=rdiscards[i]) for i=1:M]
            ...
            for i=1:M
                ...
                T_i = ITensor(t_vec, dag(inds(T1_oht[i],tags="c")[1])) * T1_oht[i]
                push!(T1_list, T_i)
            end
